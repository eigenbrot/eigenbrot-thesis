...
\begin{figure}
  ...
  \caption[Global distribution of $\tau_L$ and
    ...
    \S\ref{891_2:sec:tZ}. They are 4.3, 6.6, and \val{8}{Gyr}; and
    0.5, 0.75, 0.95, and \val{1.3}{\Zsol} for $\tau_L$ and $Z_L$,
    respectively}
\end{figure}
...
and \val{8.5}{Gyr} and we assign age bins to capture these different
...
populations. Metallicity is peaked at \val{0.75}{\Zsol}, and we choose to
...
\val{0.4}{kpc} all of the populations are young (\val{< 6}{Gyr}),
...
heights. Populations younger that \val{\asim 6}{Gyr} show the opposite
...
\val{6}{Gyr} ago, however, low-metallicity gas from outside the galaxy
...
\val{6}{Gyr} ago. Figure \ref{891_2:fig:tau_Z} confirms this. At all
radii and heights there are two distinct epochs of the enrichment
rate, $dZ_L/dt$: before \val{6}{Gyr} the disk of NGC 891 was enriched
at a rate of roughly \val{0.15}{\Zsol/Gyr}, but since \val{6}{Gyr} the
infall of pristine gas onto the galaxy has been decreasing the
metallicity at about \val{-0.18}{\Zsol/Gyr}.
...
\begin{figure}
  ...
  891]{\fixspacing\label{891_2:fig:tau_Z}Metallicity as a function of
    ...
    line denotes ``peak metallicity'' at \val{6}{Gyr}. The solid lines
...
\val{1}{kpc} the average population age asymptotes to the oldest age
(\val{\asim 10}{Gyr}). As described in \S\ref{891_2:sec:sys_err} the
...
first stars in the Universe formed \val{\asim 12}{Gyr} ago), but these
...
Figure \ref{891_2:fig:MLWA_rz} shows the projection of our data into
...
\val{0.4}{kpc} all of the populations are young (\val{< 6}{Gyr}), the
region where $0.4 < |z| \leq \val{1}{kpc}$ is a transition zone where
the age generally increases, and above \val{1}{kpc} the average
population age asymptotes to the oldest age (\val{\asim 10}{Gyr}). As
...
populations above \val{1}{kpc} is true regardless.
...
\val{8}{kpc}, which is one of the main reasons we identify this
...
estimate that at radii \val{> 8}{kpc} flaring has increased the scale
height by roughly a factor of two (up to \val{\asim 0.8}{kpc}).
...
\val{0.4}{kpc} and mostly beyond $r = \val{8}{kpc}$ and is made up of
...
In Figure \ref{891_2:fig:AV_rz} the three morphological features
...
\val{\asim 0.4}{kpc} followed by a roughly exponential decrease with
height above \val{\asim0.4}{kpc}. These results lend further proof to
the conclusion that the region below \val{0.4}{kpc} is the location of
...
% A paucity of data points above $z =
...
The metallicity trend with height, however, increases slightly up to
about \val{0.6}{kpc} at which point it decreases with increasing
...
that formed during ``peak metallicity'' around \val{6}{Gyr} ago (see
\S\ref{891_2:sec:tZ}). In this context the results of Figure
\ref{891_2:fig:MLWA_rz} take on new meaning; sequence 3 has ages
mostly between 6 and \val{8}{Gyr}, which is exactly when NGC 891 was
...
By examining trends in this pseudo-SFH space we are able to completely
...
formation more recent that \val{6}{Gyr} (i.e., anything besides the
